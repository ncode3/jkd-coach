...
def register():
    ...
    if _users_collection is None:
        return (
            jsonify({"status": "error", "message": "Authentication not available"}),
            503,
        )
...
def login():
    ...
    try:
        ...
        if not user.get("is_active", False):
            return (
                jsonify({"status": "error", "message": "User account is inactive"}),
                403,
            )
...
def log_round():
    ...
    try:
        ...
        if not payload:
            return (
                jsonify(
                    {"status": "error", "message": "Invalid or missing JSON payload"}
                ),
                400,
            )
        ...
        return (
            jsonify({"status": "error", "message": f"Invalid data type: {str(e)}"}),
            400,
        )
    except Exception as e:
...
def analyze_video():
    ...
    try:
                ...
                jsonify(
                    {
                        ...
                        "metrics": {
                            "total_frames": enriched_metrics["total_frames"],
                            "pose_coverage": round(
                                enriched_metrics["pose_coverage"] * 100, 1
                            ),
                            "guard_down_ratio": round(
                                enriched_metrics["guard_down_ratio"] * 100, 1
                            ),
                            "avg_left_guard_height": round(
                                enriched_metrics["avg_left_guard_height"], 3
                            ),
                            "avg_right_guard_height": round(
                                enriched_metrics["avg_right_guard_height"], 3
                            ),
                            "avg_hip_rotation": round(
                                enriched_metrics["avg_hip_rotation"], 1
                            ),
                            "avg_stance_width": round(
                                enriched_metrics["avg_stance_width"], 3
                            ),
                            "head_movement_score": round(
                                enriched_metrics["head_movement_score"], 3
                            ),
                        },
                        "scores": {
        ...
        return (
            jsonify({"status": "error", "message": f"Video analysis error: {str(e)}"}),
            400,
        )
    except Exception as e:
...
def generate_video_coaching(metrics: Dict[str, Any], strategy_text: str) -> str:
    ...
    if guard_down_ratio > 0.3:
        feedback_parts.append(
            f"Guard down {guard_down_ratio*100:.0f}% of the time - MAJOR concern."
        )
    elif guard_down_ratio > 0.15:
        feedback_parts.append(
            f"Guard dropping {guard_down_ratio*100:.0f}% of frames - needs work."
        )
    else:
        feedback_parts.append(
            f"Solid guard discipline ({guard_down_ratio*100:.0f}% down)."
        )

    # Pose tracking quality
    if pose_coverage < 0.5:
        feedback_parts.append(
            f"Low tracking coverage ({pose_coverage*100:.0f}%) - video quality issue or angles."
        )
    ...
    if hip_rotation < 25:
        feedback_parts.append(
            f"Hip rotation weak ({hip_rotation:.0f} degrees) - work on stance and pivots."
        )
    elif hip_rotation > 40:
        feedback_parts.append(
            f"Good hip rotation ({hip_rotation:.0f} degrees) - generating power."
        )
...
def get_rounds_history():
    ...
    try:
        ...
        # Query only the current user's rounds
        docs = list(
            _rounds_collection.where("user_id", "==", user["id"]).limit(limit).stream()
        )
        ...
        return jsonify({"rounds": rounds, "total": len(rounds)}), 200
...
def delete_round(round_id):
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "Round deleted"}), 200
